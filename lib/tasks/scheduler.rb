...
class Tasks::Scheduler
    ...
    def finalize
        ...
        @database.put({
            ...
            'params' => [{'data' => Portage3::Database::EOS}]
        })

        @logger.info("Total passed: #{(Time.now - @start_time)} seconds")
        @logger.shutdown_server
        @server_thread.kill
    end
