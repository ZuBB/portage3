...
class Package < Category
    ...
    def self.get_package_versions(atom)
        if Utils::SETTINGS['eix_presence']
            self.get_eix_versions(atom)
        else
            self.get_portage_versions(atom)
        end
    end
    ...
    def self.get_portage_versions(atom)
        tool_path = '/../../bin/list_package_ebuilds.py'
        command = File.dirname(__FILE__) + tool_path + ' ' + atom
        %x[#{command}].split("\n").map { |line|
            line[atom.size + 1..-1]
        }
    end
