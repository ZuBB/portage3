...
def get_eapi(ebuild_text)
    ...
    return get_single_line_ini_value(ebuild_text, 'EAPI').to_i || '0_EAPI_NF'
end
...
def get_slot(ebuild_text)
    ...
    return get_single_line_ini_value(ebuild_text, 'SLOT').to_i || '0_SLOT_NF'
end

def get_license(ebuild_text)
    # https://bugs.gentoo.org/show_bug.cgi?id=398575
    return get_single_line_ini_value(ebuild_text, 'LICENSE') || '0_LICENSE_NF'
end
...
def get_eapi_id(database, ebuild_obj)
    if ebuild_obj["eapi"] == '0_EAPI_NF'
        ebuild_obj["eapi"] = 0
        ebuild_obj["real_eapi"] = '0_EAPI_NF'
    end

    sql_query = "SELECT id FROM eapis WHERE eapi_version=?;"
    ebuild_obj['eapi_id'] = database.execute(sql_query, ebuild_obj["eapi"])[0][0]
...
def parse_ebuild(database, package_id, ebuild_filename)
    ebuild_text, ebuild_obj = nil, {"package_id" => package_id}
